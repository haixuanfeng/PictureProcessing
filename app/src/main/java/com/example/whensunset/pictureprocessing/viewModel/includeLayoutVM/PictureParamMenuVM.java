...
import com.example.whensunset.pictureprocessing.pictureProcessing.PictureParamMyConsumer;
...
public class PictureParamMenuVM extends ChildBaseVM {
    ...
    protected void initDefaultUIActionManager() {
        mUIActionManager = new UIActionManager(this , UIActionManager.CLICK_ACTION , UIActionManager.PROGRESS_CHANGED_ACTION);
    }
    ...
    private void initProgressChanged() {
        mUIActionManager
                .<ProgressChangedUIAction>getDefaultThrottleFlowable(PROGRESS_THROTTLE_MILLISECONDS, UIActionManager.PROGRESS_CHANGED_ACTION)
                .filter(progressChangedUIAction -> {
                    ...
                    flowable.subscribe(mat -> {
                        mEventListenerList.get(PARAM_PROGRESS_CHANGE).set(ObserverParamMap.staticSet(ObserverMapKey.PictureParamMenuVM_mat, mat));
